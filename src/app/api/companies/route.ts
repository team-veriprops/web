...
import { companies, generateCompany } from "@data/mock-companies";
...
export async function GET(req: NextRequest) {
  ...
  // Apply filters
  let filtered = companies.filter((p) => {
    let matches = true;
  ...
  // Page response
  const pageResponse = {
    items: paginated as QueryCompanyDto[],
    page,
    page_size,
...
export async function POST(req: Request) {
  ...
  const newCompany = { ...generateCompany(), ...body };
