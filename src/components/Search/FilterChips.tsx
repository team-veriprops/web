...
import { Money } from '@components/property/models';
...
export function FilterChips({ filters, onRemove, onClearAll }: FilterChipsProps) {
  ...
  if (filters.bedrooms && filters.bedrooms !== 'any') {
    ...
    if (filters.bedrooms === 'studio') {
      ...
      if (min && min === 1) bedText = `${min} bed`;
      else if (min && max && min === max) bedText = `${min} beds`;
      else if (min && max) bedText = `${min}-${max} beds`;
      else if (min) bedText = `${min}+ beds`;
    }
