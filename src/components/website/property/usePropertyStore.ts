...
export function usePropertyQueryState() {
  ...
  useEffect(() => {
    ...
    for (const [key, value] of searchParams.entries()) {
      try {
        if (value.includes(",")) {
          queryFilters[key as keyof SearchPropertyDto] = value.split(
            ","
          ) as any;
        } else if (value === "true" || value === "false") {
          queryFilters[key as keyof SearchPropertyDto] = (value ===
            "true") as any;
        } else if (!isNaN(Number(value)) && value !== "") {
          queryFilters[key as keyof SearchPropertyDto] = Number(value) as any;
        } else if (value.startsWith("{") || value.startsWith("[")) {
          queryFilters[key as keyof SearchPropertyDto] = JSON.parse(
            decodeURIComponent(value)
          );
        } else {
          queryFilters[key as keyof SearchPropertyDto] = decodeURIComponent(
            value
          ) as any;
        }
      } catch {
        queryFilters[key as keyof SearchPropertyDto] = value as any;
      }
    }
