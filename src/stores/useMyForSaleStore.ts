...
interface MyForSaleStore {
  ...
  // Actions
  setActiveTab: (tab: PropertyType) => void;
  setStatsFilter: (filter: PropertyType) => void;
    ...
    (set, get) => ({
      ...
      getFilteredListings: () => {
        ...
        let filtered = listings.filter(
          (listing) => listing.category === activeTab
        );
      ...
      getStats: () => {
        ...
        filtered = listings.filter(
          (listing) => listing.category === statsFilter
        );
